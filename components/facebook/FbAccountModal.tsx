...
import { FbAccount, FbAccountStatus, FbPerformanceLabel, Creator } from "@/types/facebook"
...
interface Props {
  ...
  creators:     Creator[]
  markets:      string[]
  onSubmit:     (account: FbAccount) => void
  onAddCreator: (name: string) => Creator
  onAddMarket:  (name: string) => string
  onClose:      () => void
}

const STATUS_OPTIONS: FbAccountStatus[]   = ["active", "scaling", "testing", "paused", "banned"]
const LABEL_OPTIONS:  FbPerformanceLabel[] = ["New", "Growing", "Top", "Stable", "Declining"]
...
export default function FbAccountModal({
  mode, account, creators, markets, onSubmit, onAddCreator, onAddMarket, onClose,
}: Props) {
  const isEdit = mode === "edit"

  const [pageHandle,       setPageHandle]       = useState(account?.pageHandle ?? "")
  const [pageName,         setPageName]         = useState(account?.pageName ?? "")
  ...
  const [status,           setStatus]           = useState<FbAccountStatus>(account?.status ?? "active")
  const [performanceLabel, setPerformanceLabel] = useState<FbPerformanceLabel>(account?.performanceLabel ?? "New")
  ...
  function validate(): boolean {
    ...
    if (!pageHandle.trim()) e.pageHandle = "Page handle is required."
  ...
  function handleAddCreator() {
    ...
    if (localCreators.some(c => c.name.toLowerCase() === name.toLowerCase())) {
      setCreatorError("Creator already exists."); return
    }
  ...
  function handleAddMarketInline() {
    ...
    if (localMarkets.some(m => m.toLowerCase() === name.toLowerCase())) {
      setMarketError("Market already exists."); return
    }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const handle       = pageHandle.trim().replace(/^@/, "")

    const result: FbAccount = {
      ...(account ?? {}),
      id:               account?.id ?? `fb_${Date.now()}`,
      pageHandle:       handle,
      ...
      <div className="relative bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-800">
          <h2 className="text-white font-semibold text-base">
            {isEdit ? "Edit Facebook Page" : "Add Facebook Page"}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-5">
          ...
          {/* Page handle + display name */}
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={LABEL_CLS}>Page Handle</label>
              <input
                type="text"
                value={pageHandle}
                ...
                className={FIELD_CLS}
              />
              <p className="text-gray-600 text-xs mt-1">facebook.com/<span className="text-gray-500">pagename</span></p>
              {errors.pageHandle && <p className="text-red-400 text-xs mt-1">{errors.pageHandle}</p>}
            </div>
            <div>
              <label className={LABEL_CLS}>Display Name (optional)</label>
              <input
                type="text"
                value={pageName}
                onChange={e => setPageName(e.target.value)}
                placeholder="e.g. Romina Garcia"
                className={FIELD_CLS}
              />
              <p className="text-gray-600 text-xs mt-1">auto-filled on first sync</p>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              {!showNewCreator && (
                <button type="button" onClick={() => setShowNewCreator(true)}
            ...
            {showNewCreator ? (
              <div className="space-y-2">
                <div className="flex gap-2">
                  <input type="text" value={newCreatorName}
                    onChange={e => { setNewCreatorName(e.target.value); setCreatorError("") }}
                    placeholder="Creator name" className={FIELD_CLS} autoFocus />
                  ...
                  <button type="button" onClick={() => { setShowNewCreator(false); setNewCreatorName(""); setCreatorError("") }}
              ...
              <select value={creatorId} onChange={e => setCreatorId(e.target.value)} className={FIELD_CLS}>
                {localCreators.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            )}
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              {!showNewMarket && (
                <button type="button" onClick={() => setShowNewMarket(true)}
                  className="text-xs text-gray-400 hover:text-white transition-colors flex items-center gap-1">
            ...
            {showNewMarket ? (
              <div className="space-y-2">
                <div className="flex gap-2">
                  <input type="text" value={newMarketName}
                    onChange={e => { setNewMarketName(e.target.value); setMarketError("") }}
                    placeholder="e.g. UK, Canada…" className={FIELD_CLS} autoFocus />
                  <button type="button" onClick={handleAddMarketInline}
                    className="px-3 py-2 bg-white text-gray-900 text-sm font-medium rounded-lg hover:bg-gray-200 transition-colors whitespace-nowrap">
                    Add
                  </button>
                  <button type="button" onClick={() => { setShowNewMarket(false); setNewMarketName(""); setMarketError("") }}
                    className="px-3 py-2 text-gray-400 hover:text-white text-sm transition-colors">
                    Cancel
                  </button>
                </div>
              ...
              <select value={market} onChange={e => setMarket(e.target.value)} className={FIELD_CLS}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={status} onChange={e => setStatus(e.target.value as FbAccountStatus)} className={FIELD_CLS}>
            ...
            <div>
              ...
              <select value={performanceLabel} onChange={e => setPerformanceLabel(e.target.value as FbPerformanceLabel)} className={FIELD_CLS}>
