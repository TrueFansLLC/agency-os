...
interface Post {
  ...
  caption: string
  video_link: string
  status: "geplant" | "gesendet" | "gepostet"
}
...
const STATUS_STYLE: Record<string, string> = {
  geplant:  "bg-gray-700 text-gray-200 border border-gray-600",
  gesendet: "bg-yellow-900/60 text-yellow-300 border border-yellow-700",
  gepostet: "bg-green-900/60 text-green-300 border border-green-700",
}
...
export default function PostingPlaner() {
  ...
  const visibleAccounts: { creator: string; account: string; platform: string }[] =
    (activeCreator === "Alle" ? Object.keys(CREATOR_ACCOUNTS) : [activeCreator])
      .flatMap(c => CREATOR_ACCOUNTS[c].map(a => ({ creator: c, ...a })))

  function getCellPosts(account: string, date: Date): Post[] {
  ...
  function openAdd(account: string, creator: string, platform: string, date: Date) {
    ...
    setModal({ mode: "add", account, creator, platform, date: dateStr, reelNumber: nextReel })
      ...
      if (modal.mode === "add") {
        await fetch("/api/posting-schedule", {
          ...
          body: JSON.stringify({
            ...
            reel_number: modal.reelNumber,
            send_date: modal.date,
    ...
    <div className="flex-1 overflow-hidden flex flex-col bg-gray-900 text-white">
      ...
      <div className="px-6 py-4 border-b border-gray-800 flex items-center justify-between gap-4 shrink-0">
        <div>
          ...
          <p className="text-sm text-gray-400">Plane Posts für alle Accounts im Voraus</p>
        </div>
        {/* Creator tabs */}
      ...
      {/* Week navigation */}
      <div className="px-6 py-3 border-b border-gray-800 flex items-center gap-4 shrink-0">
        <button
        ...
        <span className="text-sm text-gray-300 font-medium min-w-[200px] text-center">
          ...
          {addDays(weekStart, 6).toLocaleDateString("de-DE", { day: "numeric", month: "long", year: "numeric" })}
        </span>
        <button
          onClick={() => setWeekStart(d => addDays(d, 7))}
          className="text-gray-400 hover:text-white transition-colors p-1.5 rounded-lg hover:bg-gray-800"
        ...
        <button
          onClick={() => { const d = new Date(); d.setHours(0,0,0,0); setWeekStart(d) }}
          className="ml-2 text-xs text-gray-500 hover:text-gray-300 px-2 py-1 rounded hover:bg-gray-800 transition-colors"
        >
          Heute
        </button>
      </div>
          ...
          <table className="w-full border-collapse" style={{ minWidth: "900px" }}>
            <thead>
              <tr className="border-b border-gray-800">
                <th className="sticky left-0 z-10 bg-gray-900 text-left px-4 py-3 text-xs font-medium text-gray-400 uppercase tracking-wider w-40 border-r border-gray-800">
                  Account
                </th>
                {days.map(day => (
                  <th
                    key={day.toISOString()}
                    className={`px-3 py-3 text-center text-xs font-medium uppercase tracking-wider border-r border-gray-800 ${
                      isToday(day) ? "text-white bg-gray-800/50" : "text-gray-400"
                    }`}
                    style={{ minWidth: "110px" }}
                  >
                    <div className={isToday(day) ? "text-blue-400 font-semibold" : ""}>{formatDayLabel(day)}</div>
                  </th>
                ))}
              </tr>
            </thead>
            <tbody>
              {visibleAccounts.map(({ creator, account, platform }) => {
                const dotColor = CREATOR_COLORS[creator] ?? "bg-gray-500"
                return (
                  <tr key={account} className="border-b border-gray-800/60 hover:bg-gray-800/20 transition-colors">
                    <td className="sticky left-0 z-10 bg-gray-900 px-4 py-2 border-r border-gray-800">
                      <div className="flex items-center gap-2">
                        <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${dotColor}`} />
                        <span className="text-sm text-gray-200 font-medium truncate max-w-[120px]">
                          @{account}
                        </span>
                      </div>
                      {platform !== "Instagram" && (
                        <span className="text-xs text-gray-500 ml-4">{platform}</span>
                      )}
                    </td>
                    {days.map(day => {
                      ...
                      return (
                        <td
                          key={day.toISOString()}
                          className={`px-2 py-2 align-top border-r border-gray-800 ${isToday(day) ? "bg-gray-800/10" : ""}`}
                        >
                          <div className="flex flex-col gap-1">
                            {cellPosts.map(post => (
                              <button
                                ...
                                className={`w-full text-left px-2 py-1 rounded text-xs font-medium transition-colors hover:brightness-125 ${STATUS_STYLE[post.status]}`}
                              >
                                <span className="mr-1">{STATUS_ICON[post.status]}</span>
                                R{post.reel_number} · {post.send_time.slice(0, 5)}
                                {post.post_text && (
                                  <div className="text-gray-400 truncate mt-0.5 font-normal" style={{ fontSize: "10px" }}>
                            ...
                            {canAdd && (
                              <button
                                onClick={() => openAdd(account, creator, platform, day)}
                                className="w-full text-xs text-gray-600 hover:text-gray-300 hover:bg-gray-800 rounded py-1 transition-colors border border-dashed border-gray-800 hover:border-gray-600"
                              >
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setModal(null)}>
          <div className="bg-gray-900 border border-gray-700 rounded-xl w-full max-w-lg shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="px-5 py-4 border-b border-gray-800 flex items-center justify-between">
              <div>
                <h2 className="text-white font-semibold">
                  {modal.mode === "add" ? "Post hinzufügen" : "Post bearbeiten"}
                </h2>
                <p className="text-xs text-gray-400 mt-0.5">
                  @{modal.account} · R{modal.reelNumber} · {modal.date}
                </p>
              </div>
            ...
            <div className="px-5 py-4 space-y-4">
                  ...
                  <div className="flex gap-2">
                    {[1, 2, 3].map(r => (
                      <button
                        key={r}
                        onClick={() => {
                          setModal(m => m ? { ...m, reelNumber: r } : null)
                          setForm(f => ({ ...f, send_time: REEL_TIMES[r] }))
                        }}
              ...
              {/* Platform (only if IG+FB account) */}
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Worum geht es in diesem Post?"
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Caption für Instagram/Facebook..."
                  className="bg-gray-800 border border-gray-700 text-white text-sm rounded-lg px-3 py-2 w-full focus:outline-none focus:border-gray-500 resize-none placeholder-gray-600"
                />
              </div>
            ...
            <div className="px-5 py-4 border-t border-gray-800 flex items-center gap-3">
              {modal.mode === "edit" && (
                <button
                  ...
                  disabled={saving}
                  className="text-sm text-red-400 hover:text-red-300 transition-colors px-3 py-2 rounded-lg hover:bg-red-900/20"
                >
                  Löschen
                </button>
              )}
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="px-4 py-2 bg-white text-gray-900 text-sm font-medium rounded-lg hover:bg-gray-100 transition-colors disabled:opacity-50"
              >
